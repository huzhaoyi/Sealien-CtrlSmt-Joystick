 ...
 * @FilePath: include/core/smart_serial_detector.h
 * @Description: 智能串口检测器头文件
...
#ifndef SMART_SERIAL_DETECTOR_H
#define SMART_SERIAL_DETECTOR_H
...
struct SmartSerialDeviceInfo {
    ...
    std::string chip_type;            // 芯片类型（FTDI/CP210x/CH34x/CDC ACM等）
    ...
    int priority;                     // 优先级（用于多设备时的选择）
};

/**
 * @brief 串口设备匹配规则
 */
struct SerialDeviceRule {
    std::string name;                 // 规则名称
    std::string vendor_id;            // 厂商ID（可选）
    std::string product_id;           // 产品ID（可选）
    std::string serial_pattern;       // 序列号模式（可选）
    std::string driver_pattern;       // 驱动名称模式（可选）
    std::string device_pattern;       // 设备名称模式（可选）
 ...
 * @brief 智能串口检测器类
 * 
 * 该类负责智能检测和管理USB转串口设备，支持：
 * - 自动检测系统中的USB转串口设备
 * - 支持多种厂商芯片（FTDI/CP210x/CH34x/CDC ACM等）
 * - 基于规则的智能设备匹配
 * - 热插拔监控和自动恢复
 * - 动态串口路径更新
 */
...
public:
    ...
    using DeviceList = std::map<std::string, std::shared_ptr<SmartSerialDeviceInfo>>;
    using RuleList = std::vector<SerialDeviceRule>;
     ...
     * @return 设备信息列表
     */
    std::vector<SmartSerialDeviceInfo> getConnectedDevices() const;
     ...
     */
    std::shared_ptr<SmartSerialDeviceInfo> getDeviceInfo(const std::string& device_path) const;
     ...
     * @return 芯片类型字符串
     */
    std::string getDeviceChipType(const std::string& device_path) const;

    /**
     * @brief 检查设备是否支持Modbus通信
     * @param device_path 设备路径
     * @return 支持返回true，否则返回false
     */
    bool isDeviceModbusCompatible(const std::string& device_path) const;
...
private:
    ...
    mutable std::mutex rules_mutex_;              // 规则列表互斥锁
    RuleList device_rules_;                        // 设备匹配规则列表
    ...
    std::unique_ptr<USBVendorConfig> vendor_config_; // USB厂商配置
    std::string vendor_config_path_;             // 厂商配置文件路径
     ...
     * @return 设备信息指针
     */
    std::shared_ptr<SmartSerialDeviceInfo> readDeviceInfo(const std::string& device_path);
     ...
     * @param info 设备信息结构
     */
    void readUSBInfoFromSysfs(const std::string& device_path, SmartSerialDeviceInfo& info);
     ...
     * @return 优先级分数
     */
    int calculateDevicePriority(const SmartSerialDeviceInfo& device_info) const;
     ...
     */
    bool matchesRule(const SmartSerialDeviceInfo& device_info, const SerialDeviceRule& rule) const;
